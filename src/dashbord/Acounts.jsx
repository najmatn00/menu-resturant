...
const Acounts = () => {
    ...
    return ( <div className="flex flex-col items-end rounded-xl p-4 gap-2 bg-white mt-8 md:w-1/3 w-96">
    <p className="w-full text-right mb-2 text-xl font-bold">حساب کاربری</p>
     ...
     <button onClick={clickHandler} className="bg-[#D41B27] text-white w-fit px-8 py-1 rounded-xl mt-2">ویرایش</button>
     <AcountChild placeholer={user.id} stateHandler={setId}>کدملی</AcountChild>
     <button className="bg-[#D41B27] text-white w-fit px-8 py-1 rounded-xl mt-2">ویرایش</button>
    </div>);
}
